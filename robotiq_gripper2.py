u"""Module to control Robotiq's grippers - tested with HAND-E"""
...
class RobotiqGripper(object):
    ...
    ACT = u'ACT'  # act : activate (1 while activated, can be reset to clear fault status)
    GTO = u'GTO'  # gto : go to (will perform go to with the actions set in pos, for, spe)
    ATR = u'ATR'  # atr : auto-release (emergency slow move)
    ADR = u'ADR'  # adr : auto-release direction (open(1) or close(0) during auto-release)
    FOR = u'FOR'  # for : force (0-255)
    SPE = u'SPE'  # spe : speed (0-255)
    POS = u'POS'  # pos : position (0-255), 0 = open
    # READ VARIABLES
    STA = u'STA'  # status (0 = is reset, 1 = activating, 3 = active)
    PRE = u'PRE'  # position request (echo of last commanded position)
    OBJ = u'OBJ'  # object detection (0 = moving, 1 = outer grip, 2 = inner grip, 3 = no object at rest)
    FLT = u'FLT'  # fault (0=ok, see manual for errors if not zero)

    ENCODING = u'UTF-8'  # ASCII and UTF-8 both seem to work

    class GripperStatus(Enum):
        u"""Gripper status reported by the gripper. The integer values have to match what the gripper sends."""
        RESET = 0
        ACTIVATING = 1
    ...
    class ObjectStatus(Enum):
        u"""Object status reported by the gripper. The integer values have to match what the gripper sends."""
        MOVING = 0
    ...
    def connect(self, hostname, port, socket_timeout = 2.0):
        u"""Connects to a gripper at the given address.
    ...
    def disconnect(self):
        u"""Closes the connection with the gripper."""
        self.socket.close()

    def _set_vars(self, var_dict):
        u"""Sends the appropriate command via socket to set the value of n variables, and waits for its 'ack' response.
    ...
    def _set_var(self, variable, value):
        u"""Sends the appropriate command via socket to set the value of a variable, and waits for its 'ack' response.
    ...
    def _get_var(self, variable):
        u"""Sends the appropriate command to retrieve the value of a variable from the gripper, blocking until the
    ...
    def activate(self, auto_calibrate = True):
        u"""Resets the activation flag in the gripper, and sets it back to one, clearing previous fault flags.
    ...
    def is_active(self):
        u"""Returns whether the gripper is active."""
        status = self._get_var(self.STA)
        return RobotiqGripper.GripperStatus(status) == RobotiqGripper.GripperStatus.ACTIVE

    def get_min_position(self):
        u"""Returns the minimum position the gripper can reach (open position)."""
        return self._min_position

    def get_max_position(self):
        u"""Returns the maximum position the gripper can reach (closed position)."""
        return self._max_position

    def get_open_position(self):
        u"""Returns what is considered the open position for gripper (minimum position value)."""
        return self.get_min_position()

    def get_closed_position(self):
        u"""Returns what is considered the closed position for gripper (maximum position value)."""
        return self.get_max_position()

    def is_open(self):
        u"""Returns whether the current position is considered as being fully open."""
        return self.get_current_position() <= self.get_open_position()

    def is_closed(self):
        u"""Returns whether the current position is considered as being fully closed."""
        return self.get_current_position() >= self.get_closed_position()

    def get_current_position(self):
        u"""Returns the current position as returned by the physical hardware."""
        return self._get_var(self.POS)

    def auto_calibrate(self, log = True):
        u"""Attempts to calibrate the open and closed positions, by slowly closing and opening the gripper.
    ...
    def move(self, position, speed, force):
        u"""Sends commands to start moving towards the given position, with the specified speed and force.
    ...
    def move_and_wait_for_pos(self, position, speed, force):  # noqa
        u"""Sends commands to start moving towards the given position, with the specified speed and force, and
        ...
        if not set_ok:
            raise RuntimeError(u"Failed to set variables for move.")
